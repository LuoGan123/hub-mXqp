...
class SemanticMessageHistory(BaseMessageHistory):
        ...
        **kwargs,
        ...
        Args:
            ...
            redis_client (Optional[Redis]): A Redis client instance. Defaults to
                None.
            async_redis_client (Optional[AsyncRedisClient]): An async Redis client
    ...
    async def aclear(self) -> None:
        """Async clears the message history."""
    ...
    async def adelete(self) -> None:
        """Async clear all message keys and remove the search index."""
        aindex = await self._get_async_index()
    ...
    async def adrop(self, id: str | None = None) -> None:
        """Async remove a specific exchange from the message history.
        ...
        """
        if id is None:
            recent = await self.aget_recent(top_k=1, raw=True)
            id = recent[0][ID_FIELD_NAME]  # type: ignore

        aindex = await self._get_async_index()
        await aindex.client.delete(self._index.key(id))  # type: ignore
    ...
    async def acount(self, session_tag=None):
        """Async count the number of messages in the session.
        ...
        return await aindex.query(query)
    ...
    async def aget_relevant(
        ...
        """Async searches the message history for information semantically related to
        the specified prompt.
        ...
        Args:
            ...
            raw (bool): Whether to return the full Redis hash entry or just the
                message.
            role (Optional[Union[str, List[str]]]): Filter messages by role(s).
                Can be a single role string or a list of roles. If None,
        ...
        vector = await self._vectorizer.aembed(prompt)
        query = RangeQuery(
            vector=vector,
            vector_field_name=MESSAGE_VECTOR_FIELD_NAME,
            return_fields=return_fields,
        ...
        messages = await aindex.query(query)

        # if we don't find semantic matches fallback to returning recent context
        if not messages and fall_back:
            return await self.aget_recent(
                as_text=as_text, top_k=top_k, raw=raw, role=role
            )
        if raw:
            return messages
        return self._format_context(messages, as_text)
    ...
    async def aget_recent(
        ...
        """Async retrieve the recent message history in sequential order.
    ...
    async def astore(
        self, prompt: str, response: str, session_tag: str | None = None
    ) -> None:
        """Async insert a prompt:response pair into the message history.
    ...
    async def aadd_messages(
        ...
        """Async insert a list of prompts and responses into the session memory.
        ...
        for message in messages:
            content_vector = await self._vectorizer.aembed(
                message[CONTENT_FIELD_NAME]
            )
            validate_vector_dims(
                len(content_vector),
                self._index.schema.fields[MESSAGE_VECTOR_FIELD_NAME].attrs.dims,  # type: ignore
        ...
        await aindex.load(data=chat_messages, id_field=ID_FIELD_NAME)

    async def aadd_message(
        self, message: dict[str, str], session_tag: str | None = None
    ) -> None:
        """Async insert a single prompt or response into the message history.
        ...
        Args:
            ...
            session_tag (Optional[str]): Tag to be added to entry to link to a specific
                conversation session. Defaults to instance ULID.
        """
        await self.aadd_messages([message], session_tag)
