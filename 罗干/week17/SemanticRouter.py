...
class SemanticRouter(BaseModel):
    ...
    _aindex: AsyncSearchIndex | None = PrivateAttr(default=None)
    _sync_redis_client: SyncRedisClient | None = PrivateAttr(default=None)
    _async_redis_client: AsyncRedisClient | None = PrivateAttr(default=None)
    _redis_url: str = PrivateAttr(default="redis://localhost:6379")
    _connection_kwargs: dict[str, Any] | None = PrivateAttr(default=None)
    _owns_redis_client: bool = PrivateAttr(default=True)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            async_redis_client (Optional[AsyncRedisClient], optional): Async Redis client for connection. Defaults to None.
        ...
        # Store connection info for async use
        self._redis_url = redis_url
        self._connection_kwargs = connection_kwargs
        self._sync_redis_client = redis_client
        self._async_redis_client = async_redis_client
        self._owns_redis_client = redis_client is None and async_redis_client is None
    ...
    async def _get_async_redis_client(self) -> AsyncRedisClient:
        ...
        if not hasattr(self, "_async_redis_client") or self._async_redis_client is None:
            ...
            if client and isinstance(client, (SyncRedis, SyncRedisCluster)):
                self._async_redis_client = (
                    RedisConnectionFactory.sync_to_async_redis(client)
                )
            else:
                self._async_redis_client = (
                    RedisConnectionFactory.get_async_redis_connection(
                        redis_url=self._redis_url,
                        **self._connection_kwargs or {},
                    )
                )
    ...
    async def _get_async_index(self) -> AsyncSearchIndex:
        ...
        if self._aindex is None:
            ...
            self._aindex = AsyncSearchIndex(
                ...
                redis_url=self._redis_url,
                connection_kwargs=self._connection_kwargs or None,
            )
        return self._aindex

    async def _aget_route_matches(
        self,
        vector: list[float],
        aggregation_method: DistanceAggregationMethod,
        max_k: int = 1,
    ) -> list[RouteMatch]:
        """Async get route matches from vector db."""
        ...
        try:
            aggregation_result: AggregateResult = await aindex.aggregate(
                aggregate_request, vector_range_query.params
            )
        except ResponseError as e:
            if "VSS is not yet supported on FT.AGGREGATE" in str(e):
        ...
        return [
            self._process_route(route_match)
            for route_match in aggregation_result.rows
        ]

    async def _aclassify_route(
        self,
        vector: list[float],
        aggregation_method: DistanceAggregationMethod,
    ) -> RouteMatch:
        """Async classify to a single route using a vector."""
        route_matches = await self._aget_route_matches(vector, aggregation_method)
    ...
    async def _aclassify_multi_route(
        self,
        vector: list[float],
        max_k: int,
        aggregation_method: DistanceAggregationMethod,
    ) -> list[RouteMatch]:
        """Async classify to multiple routes using a vector."""
        route_matches = await self._aget_route_matches(
    ...
    async def __acall__(
        ...
        """Async query the semantic router with a given statement or vector.
        ...
        if not vector:
            if not statement:
                raise ValueError("Must provide a vector or statement to the router")
            vector = await self.vectorizer.aembed(statement)  # type: ignore

        aggregation_method = (
            aggregation_method or self.routing_config.aggregation_method
        )

        top_route_match = await self._aclassify_route(vector, aggregation_method)  # type: ignore
        return top_route_match

    @deprecated_argument("distance_threshold")
    async def aroute_many(
        self,
        statement: str | None = None,
        vector: list[float] | None = None,
        max_k: int | None = None,
        ...
        """Async query the semantic router for multiple matches.
        ...
        top_route_matches = await self._aclassify_multi_route(
            vector, max_k, aggregation_method  # type: ignore
        )
    ...
    async def aclear(self) -> None:
        """Async flush all routes from the semantic router index."""
        aindex = await self._get_async_index()
        await aindex.clear()
        self.routes = []

    async def adelete(self) -> None:
        """Async delete the semantic router index."""
        aindex = await self._get_async_index()
        await aindex.delete(drop=True)

    async def aadd_route_references(
        self,
        route_name: str,
        references: str | list[str],
    ) -> list[str]:
        """Async add reference(s) to an existing route.
        ...
        Returns:
            List[str]: The list of added reference keys.
        """
        aindex = await self._get_async_index()
        ...
        # Embed route references as a single batch
        reference_vectors = await self.vectorizer.aembed_many(references, as_buffer=True)
        ...
        keys = await aindex.load(route_references, keys=keys)
    ...
    async def aget_route_references(
        self,
        route_name: str = "",
        reference_ids: list[str] = [],
        keys: list[str] = [],
    ) -> list[dict[str, Any]]:
        """Async get references for an existing route.

        Args:
            route_name (str): The name of the route.
            reference_ids (List[str]): The reference or list of reference IDs to get.
            keys (List[str]): List of fully qualified keys.
        ...
        res = await aindex.batch_query(queries)

        return [r[0] for r in res if len(r) > 0]

    async def adelete_route_references(
        self,
        route_name: str = "",
        reference_ids: list[str] = [],
        keys: list[str] = [],
    ) -> int:
        """Async delete references for an existing route.

        Args:
            route_name Optional(str): The name of the route.
            reference_ids Optional(List[str]): The reference or list of references to delete.
            keys Optional(List[str]): List of fully qualified keys to delete.
        ...
        deleted = await aindex.drop_keys(keys)
    ...
    async def adisconnect(self) -> None:
        """Async disconnect from Redis and search index.
